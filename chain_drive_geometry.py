...
def get_first_available_value(data: dict, possible_keys: list[str]):
    """
    Return the first available value from a dictionary using a list of possible keys.

    This makes the code more robust if the CSV column names change slightly.
    """
...
def show_available_asa_sizes(catalog: pd.DataFrame) -> list[str]:
    """
    Print available ASA chain sizes from the catalog.
    """
    available_sizes = catalog["asa_size"].astype(str).str.strip().tolist()

    print("\nAvailable ASA chain sizes:")
...
def ask_asa_size(available_sizes: list[str]) -> str:
    """
    Ask the user to select an ASA chain size.
    """
    selected_asa_size = input("\nType ASA chain size, for example ASA 80 or 80: ")
    selected_asa_size = normalize_asa_size(selected_asa_size)

    if selected_asa_size not in available_sizes:
        raise ValueError(
            f"Invalid ASA size: {selected_asa_size}. "
            f"Pick one from the table: {', '.join(available_sizes)}"
        )

    return selected_asa_size
...
def plot_chain_drive(result: dict) -> None:
    """
    Plot the corrected chain drive geometry and selected chain catalog data.
...
def print_results(result: dict) -> None:
    """
    Print the main calculation results.
    """
    chain_data = result["chain_data"]
    inputs = result["inputs"]
    pitch_diameters = result["pitch_diameters"]
    ...
    print("\n=== SELECTED CHAIN ===")
    print(f"ASA size: ASA {chain_data['asa_size']}")
    print(f"Pitch P: {chain_data['pitch_P_mm']:.2f} mm")
    print(f"Pitch P: {chain_data['pitch_P_in_text']}")
    print(f"Inner width E: {chain_data['inner_width_E_mm']:.2f} mm")
    print(f"Roller diameter R: {chain_data['roller_diameter_R_mm']:.2f} mm")
    print(f"Plate height H: {chain_data['plate_height_H_mm']:.2f} mm")
    print(f"Pin diameter G: {chain_data['pin_diameter_G_mm']:.2f} mm")
    print(f"Overall width L: {chain_data['overall_width_L_mm']:.2f} mm")
    print(f"Plate thickness T: {chain_data['plate_thickness_T_mm']:.2f} mm")
    print(f"Breaking load: {chain_data['breaking_load_kgf']:.0f} kgf")
    print(f"Weight: {chain_data['weight_kg_per_m']:.2f} kg/m")
    actual_chain_length_m = chain_links["actual_chain_length_mm"] / 1000
    total_chain_weight_kg = actual_chain_length_m * chain_data["weight_kg_per_m"]

    print(f"Total chain weight: {total_chain_weight_kg:.2f} kg")

    print("\n=== INPUTS ===")
    print(f"Small sprocket teeth: {inputs['small_sprocket_teeth']}")
    print(f"Large sprocket teeth: {inputs['large_sprocket_teeth']}")
    print(
        f"Desired center distance: "
        f"{inputs['desired_center_distance_mm']:.2f} mm"
    )

    print("\n=== SPROCKET PITCH DIAMETERS ===")
    print(
        f"Small sprocket pitch diameter: "
        f"{pitch_diameters['small_pitch_diameter_mm']:.2f} mm"
    )
    print(
        f"Large sprocket pitch diameter: "
        f"{pitch_diameters['large_pitch_diameter_mm']:.2f} mm"
    )

    print("\n=== INITIAL CHAIN PATH ===")
    print(
        f"Initial chain path length: "
        f"{initial_geometry['initial_chain_path_length_mm']:.2f} mm"
    )
...
def main() -> None:
    catalog = load_chain_catalog(CSV_PATH)

    available_sizes = show_available_asa_sizes(catalog)
    selected_asa_size = ask_asa_size(available_sizes)

    chain_data = get_chain_data(catalog, selected_asa_size)

    small_sprocket_teeth = ask_positive_int(
        "\nType number of teeth for the smaller sprocket: "
    )

    large_sprocket_teeth = ask_positive_int(
        "Type number of teeth for the larger sprocket: "
    )

    desired_center_distance_mm = ask_positive_float(
        "Type desired center distance [mm]: "
    )
    ...
    try:
        ...
        print("\nInput error:")
        print(error)
        print("\nPlease check the selected chain, sprocket teeth, and desired center distance.")
        return

    print_results(result)
    plot_chain_drive(result)
